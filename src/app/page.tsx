        ...
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'100vh',padding:'20px',background:'#fff8f0'}}>
                <div style={{background:'#fff',borderRadius:'24px',padding:'40px 30px',boxShadow:'0 4px 24px rgba(0,0,0,0.1)',maxWidth:'320px',width:'100%',textAlign:'center'}}>
                          <div style={{display:'flex',justifyContent:'center',marginBottom:'8px'}}>
                                      <Image src="/mascot.png" alt="Mama's Duck mascot" width={120} height={120} style={{objectFit:'contain'}} priority />
                          </div>div>
                          <h1 style={{fontSize:'20px',fontWeight:'700',color:'#c0392b',marginBottom:'6px'}}>Mama&apos;s Duck Order</h1>h1>
                          <p style={{color:'#999',fontSize:'14px',marginBottom:'30px'}}>กรอก PIN 4 หลัก</p>p>
                  ...
                  {error && <p style={{color:'#e74c3c',fontSize:'13px',marginBottom:'16px'}}>{error}</p>p>}
                  {loading && <p style={{color:'#999',fontSize:'13px',marginBottom:'16px'}}>กำลังตรวจสอบ...</p>p>}
                      ...
                      <button key={i} onClick={() => { if(d===String.fromCharCode(8592)){setPin(p=>p.slice(0,-1));setError('');}else if(d)handlePin(d); }}
                                      ...
                                      disabled={loading || !d}>{d}</button>button>
                    ))}
                          </div>div>
                </div>div>
        </div>div>
